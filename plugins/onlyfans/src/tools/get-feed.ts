import { defineTool } from '@opentabs-dev/plugin-sdk';
import { z } from 'zod';
import { api } from '../onlyfans-api.js';
import { type RawPost, mapPost, postSchema } from './schemas.js';

interface FeedResponse {
  list?: RawPost[];
  hasMore?: boolean;
  tailMarker?: string;
}

export const getFeed = defineTool({
  name: 'get_feed',
  displayName: 'Get Feed',
  description:
    'Get the main content feed with posts from subscribed creators. Returns posts in reverse chronological order with pagination support.',
  summary: 'Get your content feed',
  icon: 'rss',
  group: 'Feed',
  input: z.object({
    limit: z.number().int().min(1).max(50).optional().describe('Number of posts to return (default 10, max 50)'),
    offset: z.string().optional().describe('Pagination marker from a previous response (tail_marker)'),
  }),
  output: z.object({
    posts: z.array(postSchema).describe('Feed posts'),
    has_more: z.boolean().describe('Whether more posts are available'),
    tail_marker: z.string().describe('Pagination marker for the next page'),
  }),
  handle: async params => {
    const query: Record<string, string | number | boolean | undefined> = {
      limit: params.limit ?? 10,
      skip_users: 'all',
      format: 'infinite',
    };
    if (params.offset) query.afterPublishTime = params.offset;

    const data = await api<FeedResponse>('/posts', { query });
    return {
      posts: (data.list ?? []).map(mapPost),
      has_more: data.hasMore ?? false,
      tail_marker: data.tailMarker ?? '',
    };
  },
});
