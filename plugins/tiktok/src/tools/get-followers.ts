...
export const getFollowers = defineTool({
  ...
  description:
    "Get the list of followers for a TikTok user. Requires the user's secure user ID (secUid). Use get_user_profile to find a user's secUid from their username. Supports cursor-based pagination.",
  ...
  output: z.object({
    users: z.array(listUserSchema).describe('List of followers'),
    ...
    total: z.number().int().describe('Total number of followers'),
  }),
  handle: async params => {
