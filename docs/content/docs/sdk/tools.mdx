...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>`string`</Table.Cell>
      <Table.Cell>Human-readable name shown in the side panel (e.g., `'Send Message'`).</Table.Cell>
    </Table.Row>
    <Table.Row>
    ...
    <Table.Row>
      ...
      <Table.Cell>Lucide icon in kebab-case (e.g., `'message-square'`). Displayed in the side panel. See [lucide.dev/icons](https://lucide.dev/icons).</Table.Cell>
    </Table.Row>
    <Table.Row>
...
interface ProgressOptions {
  progress: number;  // Current value (e.g., 3 of 10)
  total: number;     // Total expected value (e.g., 10)
...
export const sendMessage = defineTool({
  ...
  async handle(params, context?: ToolHandlerContext) {
    const result = await fetchJSON<{ ok: boolean; ts: string }>(
