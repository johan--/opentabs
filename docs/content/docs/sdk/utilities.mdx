...
```typescript
function deleteJSON<T>(url: string, init?: FetchFromPageOptions): Promise<T>;
...
Maps an HTTP error response to a `ToolError` with the appropriate category. Automatically classifies status codes: 401/403 → `auth`, 404 → `not_found`, 429 → `rate_limit` (with `Retry-After` header parsing), everything else → `internal`.
